...
ifstream input_gen(testcase_input);
...
void read_data(){
    ...
    for (int i = 1; i <= num_fars; i++){
        input_gen >> grid[i].lat >> grid[i].lon;
    }

    input_gen >> num_vertices;
    vertices.resize(num_vertices + 2);
    for (int i = 1; i <= num_vertices; i++){
        input_gen >> vertices[i].lat >> vertices[i].lon;
    }
    ...
    input_gen >> num_graverows;
    grave_rows.resize(num_graverows + 2);
    for (int i = 1; i <= num_graverows; i++){
        grave_row_type gr;
        input_gen >> gr.area_name;
        input_gen >> gr.far_west >> gr.far_east;
        grave_rows[i] = gr;
    }

    input_gen >> num_suggest;
...
int main(){
    ...
    input_gen >> testcase.coor.lat >> testcase.coor.lon >> testcase.row_id;
    read_data();
